...
describe("API testing", function() {
  ...
  it("View All Contacts", function(done) {
    ...
    collectionStub
      .withArgs("contacts")
      .returns({ get: () => Promise.resolve(contactList) });
  ...
  describe("Add new contact", function() {
    beforeAll(() => {
      ...
      collectionStub
        .withArgs("contacts")
        .returns({
          add: (contact: any) => Promise.resolve({ id: 1, get: () => contact })
        });
    });
      ...
      request(webApi)
        ...
        .end((err, res) => {
          expect(res.text).toMatch(
            "Contact should only contains firstName, lastName and email!!!"
          );
          done();
        });
    });
  ...
  describe("View a contact", function() {
    beforeAll(() => {
      ...
      collectionStub.withArgs("contacts").returns({
